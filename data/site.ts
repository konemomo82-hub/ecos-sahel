...
export const antennas = {
  mali: {
    ...
    // Plaquette : texte suivi de présentation. Les chiffres proviennent du rapport
    // moral 2024 déjà résumé dans les missions ci-dessous.
    plaquette_fr: [
    ...
    plaquette_en: [
      "ECOS Mali is based in Heremankono, on the outskirts of Bamako, where it has worked since 2002. Twenty years in the same place changes the nature of the work: you no longer arrive offering projects, you know the families. The children who first read under the mango trees now supervise the younger ones. That continuity is the association's greatest asset, well ahead of its funding.",
      "It all started with books. In many Malian neighbourhoods, reading means travelling, registering, sometimes paying — three obstacles too many for a family counting every franc. ECOS Mali turned the problem around and brings the library to where the children already are: in the shade of the mango trees, in the middle of the neighbourhood, with no formalities. In 2024 that meant forty-eight sessions and around fifty children followed regularly, six in ten of them girls, supported by five young volunteers from the neighbourhood.",
      "Then comes the question that arises in adolescence: stay or leave. Many young people leave their village for lack of any future there. The association trains them in land-based trades — fish farming, organic market gardening, local food processing — not as a fallback, but as work you can actually live on. In 2024, fifty-two young people in precarious situations went through the programme, thirty-four of whom had fled violence. Nearly two in three have since started a micro-business of their own.",
      "Both of these would soon hit a wall without the third strand. Repeated droughts, exhausted soils, rains that have become unpredictable: in the Sahel, climate is not a distant concern, it is the frame of daily life. So ECOS Mali trains community facilitators in agroecology, water management and reforestation. Ten of them were certified at the Tiebani workshop in September 2024, with one clear instruction: this knowledge must stay put and be passed on without us.",
      "What holds it all together is the last strand. A neighbourhood where people no longer speak to each other does not learn, does not farm and does not rebuild. The association opens spaces where young people, women and customary authorities settle their disputes and build projects together, drawing on forms of mediation that long predate it. Under the presidency of Esaïe Kamaté, ECOS Mali keeps moving this way: slowly, close to the ground, with local people rather than on their behalf.",
    ],
    missions_fr: [
      {
  ...
  burkina: {
    ...
    // Plaquette : texte suivi de présentation.
    plaquette_fr: [
    ...
    plaquette_en: [
      "ECOS Burkina Faso is based in Ouagadougou. The association works in a place where two emergencies overlap: land growing poorer year after year, and thousands of families displaced by the security crisis. It chose not to treat them separately, because on the ground they are not separate.",
      "The first strand is the land. The desert is advancing, soils are losing their fertility, and the country has launched a national reforestation effort, the Green Belt. ECOS Burkina Faso contributes at its own scale: growing forest and fruit seedlings suited to the local climate, then organising planting days with schools and neighbourhoods. The aim is not only to put trees back in the ground. It is also that a child who planted one looks at it differently ten years later.",
      "The second strand answers a more immediate question: how do you keep farming when land is scarce and water is running short? The association trains growers, women and young people in greenhouse and soil-free cultivation. These methods save water, shield harvests from bad weather and make year-round production possible on small plots, in town as well as in the countryside. For a family, that means an income no longer riding entirely on a single season.",
      "The third strand is human. The humanitarian situation remains heavy, and many families arrive with nothing in neighbourhoods already under strain. ECOS Burkina Faso provides concrete support — food, essential kits — but focuses above all on what comes after the emergency: creating occasions to meet, such as Kaya Nooma, where displaced families and host communities encounter one another somewhere other than a distribution queue.",
      "Under the presidency of Ibrahima Koné, the association holds these three strands together. Planting, producing, welcoming: three ways of saying that a territory cannot be repaired one piece at a time.",
    ],
    missions_fr: [
      {
